...
body = json.dumps({"Cmd": ["sh", "-c", "ls -la /app/dependencies/ > /tmp/p.log 2>&1; echo --- >> /tmp/p.log; ls -la /app/*.jar >> /tmp/p.log 2>&1; echo --- >> /tmp/p.log; ps aux | grep java >> /tmp/p.log 2>&1"]}).encode()
...
print(data[512:512+sz].decode('utf-8', errors='replace')[:2000])
