...
pub async fn handle(
    ...
    if blob.is_empty() {
        // MS-SMB2 §3.3.5.2 step 1: smbj 0.14 sends SESSION_SETUP with an
        // empty security buffer when negotiating. Treat it as the wild-card
        // first round and let the NTLM acceptor generate its NEGOTIATE.
        tracing::debug!("session_setup first round with empty blob; treating as NEGOTIATE");
    }
    ...
    let (outcome, raw_form) = {
        ...
        let outcome = match acceptor.authenticate(&inner_token, lookup) {
            Ok(o) => o,
            Err(e) => {
                tracing::warn!(error = %e, "NTLM authenticate failed");
                return HandlerResponse::err(ntstatus::STATUS_LOGON_FAILURE);
            }
        };
        (outcome, raw_form)
