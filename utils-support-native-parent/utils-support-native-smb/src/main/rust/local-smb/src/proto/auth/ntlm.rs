...
impl NtlmServer {
        ...
        F: Fn(&str, &str) -> Option<UserCreds>,
    {
        ...
        // ---- Anonymous fast path. MS-NLMP §3.2.5.1.2: empty user (with or
        // without an NT response — smbj 0.14 sends a non-empty NT response
        // even for anonymous, so we check only the user field) means
        // anonymous logon.
