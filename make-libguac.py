"""Run make in guacamole-server to capture real error"""
...
# Try to make just the libguac library and capture error
print("=== make -C src/libguac ===")
out, err = docker_exec_sync(["sh", "-c", "cd /root/.utils-support-gateway/cache/guacd/1.5.5/guacamole-server-1.5.5 && make -C src/libguac 2>&1 | tail -40"], timeout=120)
print("STDOUT:", out)
print("STDERR:", err)

print("\n=== check string.c exists ===")
out, err = docker_exec_sync(["sh", "-c", "ls -la /root/.utils-support-gateway/cache/guacd/1.5.5/guacamole-server-1.5.5/src/libguac/string.c 2>&1"], timeout=10)
print(out)
