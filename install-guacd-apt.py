"""Try apt-get install guacd in container"""
...
# Try apt-get install guacd
print("=== apt-get install guacd ===")
out, err = docker_exec_sync(
    ["sh", "-c", "apt-get install -y guacd 2>&1 | tail -30"],
    timeout=120)
print("STDOUT:", out)
print("STDERR:", err)

# Check if installed
print("\n=== which guacd after install ===")
out, err = docker_exec_sync(["sh", "-c", "which -a guacd 2>&1"], timeout=10)
print(out)

print("\n=== find SSH libs after install ===")
out, err = docker_exec_sync(["sh", "-c", "find / -name 'libguac-client*' 2>/dev/null | head"], timeout=15)
print(out)
