"""
Test ssh from container with key auth, since password might not work
Actually let's just check the sshd log
"""
import urllib.request, json, time
URL = 'http://172.16.0.40:2375'

# Check sshd log
body = json.dumps({"Cmd": ["sh", "-c", "cat /tmp/sshd.log > /tmp/p.log 2>&1; ls -la /tmp/sshd.log >> /tmp/p.log 2>&1"]}).encode()
