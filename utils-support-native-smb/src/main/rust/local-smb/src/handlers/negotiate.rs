...
use crate::proto::messages::{
    Dialect, EncryptionCapabilities, NegotiateContext, NegotiateRequest, NegotiateResponse,
    PreauthIntegrityCapabilities, SigningCapabilities,
};
use tracing::info;
use uuid::Uuid;
...
pub async fn handle(
    ...
    *conn.signing_algo.write().await = match dialect {
    ...
    if dialect == Dialect::Smb311 {
        ...
        // ENCRYPTION_CAPABILITIES — smbj 0.14 only recognises types 0x0001..0x0005.
        // We previously advertised SIGNING (0x0008) which smbj rejected with NPE.
        ...
        let ctxs = vec![preauth_ctx, encryption_ctx];
